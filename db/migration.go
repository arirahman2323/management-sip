	...
	migrations := []struct {
		...
		},
		{
			...
			CREATE TABLE IF NOT EXISTS products (
				...
				FOREIGN KEY (unit_id) REFERENCES unit_types(id) ON DELETE CASCADE
			);`,
		},
		{
			name: "products_in",
			query: `
				CREATE TABLE IF NOT EXISTS products_in (
					id TEXT PRIMARY KEY,							-- ID sistem (e.g. PIN-001)
					product_id TEXT NOT NULL,						-- ID produk (TEXT, FK ke products)
					quantity INTEGER NOT NULL,						-- Jumlah produk masuk
					supplier TEXT,									-- Nama supplier (opsional)
					created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
					updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
		...
		},
		{
			...
			query: `
				CREATE TABLE IF NOT EXISTS products_out (
					id TEXT PRIMARY KEY,					-- ID sistem (e.g. POUT-001)
					product_id TEXT NOT NULL,				-- ID produk (TEXT, FK ke products)
					quantity INTEGER NOT NULL,				-- Jumlah produk keluar
					receiver TEXT,							-- Nama penerima (opsional)
					note TEXT,								-- Catatan tambahan
					created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
					updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
