...
const Index = () => {
  ...
  const fetchProductIn = async () => {
    try {
      const [productInRes, masterProductRes] = await Promise.all([axiosInstance.get(API_PATHS.PRODUCT_IN.GET_PRODUCT_IN), axiosInstance.get(API_PATHS.PRODUCT.GET_PRODUCT)]);

      const productInList = productInRes?.data || [];
      const masterProductList = masterProductRes?.data?.data || [];

      const productMap = new Map(masterProductList.map((product) => [product.id, product]));
      ...
      <DashboardLayout>
        <div className="p-6 min-h-full">
          ...
          <div className="flex flex-col bg-white p-4 rounded-lg shadow-md">
            ...
            {/* Tabel */}
            <DataTable columns={columns} data={filteredData} pagination highlightOnHover noDataComponent={<div className="text-center text-gray-500 p-4">Tidak ada data produk masuk.</div>} />
          </div>
        </div>
      </DashboardLayout>
