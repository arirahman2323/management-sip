    ...
    const fetchDashboardData = async () => {
      ...
      try {
        const res = await axiosInstance.get(
          API_PATHS.DASHBOARD.GET_DASHBOARD(selectedYear)
        );
    ...
    datasets: [
      {
        ...
        data: monthLabels.map((_, index) => {
          const monthData = (dashboardData?.product_in?.monthly || []).find(
            (m) => parseInt(m.month, 10) === index + 1
          );
      ...
      },
      {
        label: "Produk Keluar",
        data: monthLabels.map((_, index) => {
          const monthData = (dashboardData?.product_out?.monthly || []).find(
            (m) => parseInt(m.month, 10) === index + 1
          );
          return monthData ? monthData.total_quantity : 0;
        }),
    ...
    datasets: [
      {
        label: "Jumlah Produk",
        data: [
          dashboardData?.product_in?.yearly_total || 0,
          dashboardData?.product_out?.yearly_total || 0,
        ],
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center bg-white rounded-md">
            ...
            <select
              ...
              className="cursor-pointer text-gray-600 rounded-sm p-2 bg-white text-sm font-medium focus:outline-none"
            >
              {years.map((year) => (
          ...
          <div className="lg:col-span-2 flex flex-col gap-6 mb-4">
            ...
            <div className="lg:col-span-3 bg-white p-6 rounded-lg shadow-md">
              <h3 className="text-lg font-semibold mb-4">
                Grafik Barang Masuk dan Keluar ({selectedYear})
              </h3>
                  ...
                  <div className="ml-4">
                    <p className="text-xl font-bold text-green-500">
                      {isLoading
                        ? "..."
                        : formatRupiah(
                            dashboardData?.summary?.total_transaction
                          )}
                    </p>
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <p className="text-xl font-bold text-green-500">
                      {isLoading
                        ? "..."
                        : formatRupiah(dashboardData?.summary?.total_profit)}
                    </p>
